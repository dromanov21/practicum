...
int occur_elem(struct node *head, char elem)
{
    int a = 0;
...
void make_list(struct node *head1, struct node *head2, struct node *head3){
    ...
    {
        if (occur_elem(head2, head1->data) == 0)
        {
    ...
    {
        if (occur_elem(head1, head2->data) == 0)
        {
...
int main(void){
    ...
    printf("Enter elements to first list: ");
    scanf("%c", &(head1->data));
    while (head1->data != '.'){
